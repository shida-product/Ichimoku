...
**side-peek の操作ボタンを上部へ移動（本セッション 2026-06-18・最新）**: スライドイン詳細パネルの保存/削除などの操作ボタンが最下部で見づらい、との指摘に対応。共通枠 `src/features/_shared/panel.tsx` の `PanelShell` に **`actions` prop（ヘッダ直下に固定表示する操作行）** を新設し、本体スクロールに依らず常に見える上部へ操作ボタンを集約。`EventDetailPanel`（削除/キャンセル/保存・更新）と `TaskDetailPanel`（削除/未着手へ戻す）を `footer` → `actions` へ移行。タスクの作成/更新時刻は補足情報として `footer`（最下部）に残置。説明テキストのみのパネル（`CategoryManager`/`ShiftManager`=ManageListPanel、`CompletedHistory`）は従来どおり `footer` 下部のまま（操作ボタンを持たないため変更なし）。`PanelFooterRow` の左右レイアウト部品はそのまま流用。検証: `npm.cmd run lint` 0 error（既存 Fast Refresh warnings のみ）/ `npm.cmd run build`（tsc 込み）成功 / 対象3ファイル Prettier クリーン。⚠ 実機目視推奨: 狭い幅（390px）でヘッダ行に削除/キャンセル/保存＋✕が収まるか。
