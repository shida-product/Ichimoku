...
**バックエンド整備：専用スキーマ隔離＋共有プロジェクトのセキュリティ監査（本セッション 2026-06-19・最新）**: バックエンド着手にあたり、Supabase が**他アプリ（ringo_\* / ogi_\* / reception_tickets / questionnaire_responses / guests）と同一プロジェクトに同居**していることが判明。①**Ichimoku を専用スキーマ `ichimoku` に隔離**＝3マイグレーション（`20260615.../20260616.../20260618...`）を全面書き換え（テーブル・RLS・トリガー関数 `set_updated_at()` をスキーマ内へ。`public` の関数上書き事故を解消）。GRANT は `authenticated`/`service_role` のみ（**anon 除外**）。`src/lib/supabase.ts` を `createClient<any,"ichimoku">(..., { db:{ schema:"ichimoku" } })` に。`.gitignore` に CLI 生成物追加。検証: lint 0 error / build 成功。**⚠ 実DB未適用**（CLI 未導入・`.env.local` なし）。残りユーザー手順: `.env.local` 設定 → `npx supabase init/link/db push`（`config push` は禁止＝他アプリの API 設定を壊す）→ ダッシュボードで Exposed schemas に `ichimoku` 追加。②**他アプリのセキュリティ監査を実施**＝`public` 全テーブルが anon にフル GRANT、かつ ogi_\*/reception/ringo_\*/questionnaire_responses が `using(true)` 素通しで**個人情報がネット全公開状態**（🔴）。`guests` のみ適切（🟢）。修正は**後日・別セッション**で実施と決定。手順・修正SQL・リスク・ロールバック・不足情報を **[docs/supabase-security-audit.md](../../docs/supabase-security-audit.md)** に集約（①questionnaire ②ringo は情報充足で実行可、③ogi はログインメール未確認、④reception は端末操作/PII 要確認）。データ消失リスクは無し（権限変更のみ・可逆）、アプリ停止リスクは「今 anon でやっている操作を取り上げると壊れる」点に注意。

**方針A: 優先度・予定色を撤回し締切時刻のみ（本セッション 2026-06-18・最新）**: 「色や優先度の設定そのものが整理作業になり、北極星（整理せず消化）と矛盾する」とのユーザー指摘を受け、フェーズ1で入れた**①タスク優先度②予定の手動色分けを撤回**。**消化に直結する③締切時刻のみ残す**。`types.ts` から `TaskPriority`/`Task.priority`/`EventItem.color` を削除（`Task.dueTime` は維持）。マイグレーション `20260618000000_task_event_fields.sql` は `tasks.due_time` のみへ縮小。`AppDataContext`・`mockData`・`TaskDetailPanel`（優先度UI削除・時刻入力維持）・`TaskCard`（`PriorityMark` 削除・締切チップ時刻併記維持）・`EventDetailPanel`（色UI削除）・`Agenda`（色アクセント撤回＝既定固定色に復帰）を巻き戻し。ADR-0002 を「方針A」で改訂し、**「整理を生まない原則」**（既定無設定で成立／仕分け系は持たせない／消化直結シグナルを自動で）を明文化。検証: `npm.cmd run lint` 0 error / `npm.cmd run build` 成功 / Prettier クリーン。⚠ 実 DB 未適用（migration 未適用・認証情報なし）。**タスク詳細に完了ボタン追加（本セッション 2026-06-18・最新）**: これまで完了は「完了ゾーンへドラッグ」のみだったが、スライドイン（`TaskDetailPanel`）の操作行に**「完了」ボタン**を追加（未完了タスクのみ表示・押下で `completeTask`＝即アーカイブ→パネルを閉じる）。完了履歴から開いた完了タスクは従来どおり「未着手へ戻す」。削除は常時。検証: lint 0 error / build 成功 / Prettier クリーン。

**列分配を可変マソンリー化＋未分類4列＋レーン件数復活（本セッション 2026-06-18）**: ①`Board.tsx` のカテゴリ列分配を round-robin（index%列数）から **高さ貪欲マソンリー**（各カテゴリを「その時点で最短の列」へ入れる）に変更。高さは内容から推定（`laneWeight`＝見出し＋カード＋メモ＋締切チップ、折りたたみ時は見出しのみ）。これで 5 つ目が固定で2列目下でなく最短列（例:3列目）へ潜り、列高が揃いやすい。②未分類レーンのグリッドをカテゴリ列と同じ**レスポンシブ列数（colCount・最大4＝MAX_COLS）**に（`BoardCell` に `gridCols` を追加し `grid-cols-3` 固定→ inline `gridTemplateColumns` 可変、`Lane` 経由で受け渡し）。③**レーン見出しの件数表示を復活**（タスク溢れ把握のため・`tasks.length`）。注: 管理パネル（ManageListPanel）の使用数は引き続き非表示のまま。検証: lint 0 error / build 成功 / Prettier クリーン。実機目視推奨（列バランス・未分類4列・折りたたみ時の再配置）。

**モック充実・実機想定（本セッション 2026-06-18）**: `mockData.ts` を拡充。カテゴリを **5 つ**（営業/開発/経理/採用/総務）＋未分類に増やし、タスクを各カテゴリで不揃い（営業4/開発6/経理2/採用3/総務1/未分類2＝計18 active＋完了3）にして**列分配マソンリーの列高さがバラつく**よう設計。メモ有無・複数リンク・締切〔日付/時刻〕・★（doing）も散らしカード高さも変化。締切は超過(6/10,6/12)〜今週〜先(6/30)まで分散し締切カラム/カレンダー描き分けを確認しやすく。予定は 6/15〜6/26 に拡充（会食・経営会議・終日・複数日またぎ含む）、勤務地割当も 6/24 まで延長。検証: lint 0 error / build 成功 / Prettier クリーン。

**カレンダー曜日色＋祝日判定（本セッション 2026-06-18）**: アジェンダ各日の曜日・日付の文字色を **土曜＝青（`text-primary`）／日曜・祝日＝赤（`text-crit`）／平日＝既定** に。今日は従来どおり青丸で強調（曜日色より優先）。祝日は外部依存を増やさず新規 `src/lib/holidays.ts`（`isHoliday`）で自前判定＝固定祝日＋ハッピーマンデー＋春分/秋分（1980–2099 近似式）＋振替休日＋国民の休日、年単位メモ化。天皇誕生日は 2/23（2020 以降）採用。検証: lint 0 error / build 成功 / Prettier クリーン。**祝日データ方針（決定）**: 公式CSV同梱や外部APIは採用せず、**将来の Google カレンダー連携（Step 12-13）で Google の「日本の祝日」カレンダーから取得**する。現状の `holidays.ts`（自前計算）は**それまでの暫定**＝通常年は正確だが一時的祝日・遠い将来の春分秋分エッジは追えない点に留意。⚠ 暫定期間は主要日（春分・秋分・振替）を実機で要目視。

**各所の合計数値を撤去（本セッション 2026-06-18）**: 「整理を生まない／数値で急かさない」方針に沿い、件数表示を削除。①ボードのレーン見出しの件数（`Lane.tsx` の `tasks.length` 表示）。②カテゴリ/勤務地管理の使用数（`ManageListPanel` の `countOf` 表示）＝`countOf` prop ごと撤去し、`CategoryManager`/`ShiftManager` の `countOf` 算出と不要になった `tasks`/`shifts` 参照も整理。検証: lint 0 error / build 成功 / Prettier クリーン。

**ファビコンは当初の紫グリフへ差し戻し（本セッション 2026-06-18）**: 「一」モノグラム及び6案コンペ（`public/favicon-lab/`）はユーザー判断で不採用。`public/favicon.svg` を当初の紫グリフ（git 履歴 `c364946^` から復元）に戻し、`favicon-lab` 一式を削除。`index.html` の `<title>` は「Ichimoku」のまま維持（戻さない）。⚠ 現行ファビコンは紫（#863bff）でアプリの Google Blue 配色とは不一致だが、ユーザー意向により当面これを採用。

**仕様書更新＋README作成（本セッション 2026-06-18）**: ここまでの実装を仕様へ反映。`task-board-spec-v1.md` を **v1.5** へ更新（締切 `due_time` 採用＝tasks 表＋§3.3.3＋§10、勤務地への表記統一＝§3.8、3カラム＋設定ボタン配置＝§7、カレンダーの予定/締切の自動描き分け＝§3.5、優先度/予定色の不採用＝ADR-0002 参照、ルーティング行を「なし＝認証ゲーティング」に修正、フッター更新）。`docs/design.md` に設定ボタン配置・カレンダー描き分け・予定の保存ボタン例外/操作ボタン位置を追記。**ルート `README.md` を新規作成**（概要・北極星・3カラム・技術スタック・セットアップ/環境変数/プレビューモック・ディレクトリ構成・機能・ドキュメント索引・ロードマップ・`.agents` 運用）。コード変更なし（ドキュメントのみ）。Prettier 整形済み。

**UI微調整一括（本セッション 2026-06-18）**: ①完了ゾーンの件数表示を撤去（`CompleteZone` の `totalCount` prop と `Board` の `archivedTasks` 参照も整理）。②勤務地の **UI表記を「勤務地」に統一**（"シフト"の語を画面から除去・内部テーブル名 `shift_types`/`shifts` は据え置き）。③**設定ボタンを効く列のヘッダーへ**: カテゴリ管理を全体ヘッダー→タスクボードのヘッダーへ移動（全体ヘッダーはタイトル＋アカウントのみ）、勤務地はカレンダーヘッダーのまま。④**勤務地チップを各日の日付の真下**へ縦置き（日付列を `w-[4.5rem]` に拡幅、`ShiftChip` を `max-w-full`＋name truncate で列幅に収める）、**左の色ドットは撤去**（チップ自体が色付きで冗長）。⑤**ファビコンを刷新**: 旧＝中身と無関係な紫の抽象グリフ（テンプレ流用）→ Google Blue 地に「一」（＝一目/Ichimoku）を白い角丸バーで描いた `public/favicon.svg`（フォント非依存・極小でも視認可）。`index.html` の `<title>` を「Ichimoku」に簡素化。各検証: lint 0 error / build 成功 / Prettier クリーン。
...
|  4   | Step 12-13. Google アカウント連携＋双方向同期（`docs/google-calendar-setup.md`）。**祝日も Google の「日本の祝日」カレンダーから取得**（暫定の自前 `holidays.ts` を置換）|  ☐   |
...
|  9   | バックエンド整備: Ichimoku を専用スキーマ `ichimoku` に隔離（3 migration 書換＋client `db.schema` 済）。残: `.env.local` 設定 → `npx supabase init/link/db push` → ダッシュボードで Exposed schemas に `ichimoku` 追加 → CRUD/RLS/TZ 検証。本番化（`APP_TODAY` 実日付化・mutation 失敗トースト）も残 |  ◐   |
|  10  | **他アプリのセキュリティ修正（別セッション）**: 共有 Supabase の `public` 同居アプリが anon 全公開（🔴）。手順・SQL・リスクは [docs/supabase-security-audit.md](../../docs/supabase-security-audit.md)。①questionnaire ②ringo は実行可、③ogi=メール未確認 ④reception=端末操作/PII 要確認 |  ☐   |
